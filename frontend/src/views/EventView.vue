  ...
  <div class="home">
    ...
    <div v-else-if="eventError" class="error-container">
      <div class="error-card">
        <h2>このURLは現在無効です</h2>
        <p>{{ eventError }}</p>
        <p class="error-hint">イベントが終了したか、URLが無効になっています。</p>
      </div>
    </div>
    ...
    <div v-else class="container">
      ...
      <section v-if="receiptNumber" class="receipt-section">
        <div class="receipt-card">
          ...
          <div class="receipt-preview">
            <img :src="canvasLicenseUrl" alt="Pet License" class="receipt-license-image" />
          </div>

          <div class="receipt-actions">
            <a :href="canvasLicenseUrl" :download="`${formData.pet_name || aiPlaceholders.pet_name || 'pet'}_license.png`" class="btn btn-download">
              免許証をダウンロード
            </a>
            <button @click="reset" class="btn btn-secondary">新しい免許証を作成</button>
          </div>
        </div>
      </section>
      ...
      <section v-else-if="canvasLicenseUrl" class="license-section no-header">
        <p class="click-instruction">入力内容を変更すると免許証がリアルタイムで更新されます。画像をクリックして編集項目に移動できます。</p>
        <div class="license-image-container">
          <img ref="licenseImageRef" :src="canvasLicenseUrl" alt="Pet License" class="license-image" />
          <!-- クリック可能エリアのオーバーレイ -->
          <div class="click-overlay" :class="{ 'show-hint': showClickHint }">
            <div class="click-area pet-name-header" @click="scrollToField('pet_name')" title="ペット名を編集"></div>
            <div class="click-area birth-date" @click="scrollToField('birth_date')" title="生年月日を編集"></div>
            <div class="click-area gender" @click="scrollToField('gender')" title="性別を編集"></div>
            <div class="click-area breed" @click="scrollToField('breed')" title="種類を編集"></div>
            <div class="click-area color" @click="scrollToField('color')" title="毛色を編集"></div>
            <div class="click-area owner-name" @click="scrollToField('owner_name')" title="飼い主名を編集"></div>
            <div class="click-area favorite-word" @click="scrollToField('favorite_word')" title="お好きな一言を編集"></div>
            <div class="click-area microchip" @click="scrollToField('microchip_no')" title="マイクロチップNoを編集"></div>
            <div class="click-area special-notes" @click="scrollToField('special_notes')" title="特記事項を編集"></div>
          </div>
        </div>
        ...
        <div class="license-actions">
          ...
          <div class="download-buttons">
            <a :href="canvasLicenseUrl" :download="`${formData.pet_name || 'pet'}_license.png`" class="btn btn-download">
              ダウンロード
            </a>
      ...
      <section v-else class="upload-section">
        <div class="upload-hero">
          ...
          <p class="hero-subtitle">かわいいペットの写真をアップロードして、オリジナル免許証を作成</p>
        </div>
      ...
      <section v-if="canvasLicenseUrl && !receiptNumber" class="form-section">
        ...
        <div class="form-grid">
          <div class="form-group" ref="field_pet_name">
            ...
            <input v-model="formData.pet_name" type="text" :placeholder="aiPlaceholders.pet_name || 'ポチ'" :class="{ 'ai-placeholder': !formData.pet_name && aiPlaceholders.pet_name }" />
          </div>
          ...
          <div class="form-group" ref="field_breed">
            <label>種類</label>
            <input v-model="formData.breed" type="text" :placeholder="aiPlaceholders.breed || 'ゴールデンレトリバー'" :class="{ 'ai-placeholder': !formData.breed && aiPlaceholders.breed }" />
          </div>

          <div class="form-group" ref="field_color">
            <label>毛色</label>
            <input v-model="formData.color" type="text" :placeholder="aiPlaceholders.color || 'ブラック'" :class="{ 'ai-placeholder': !formData.color && aiPlaceholders.color }" />
          </div>

          <div class="form-group" ref="field_owner_name">
            <label>飼い主名（保護者欄）</label>
            <input v-model="formData.owner_name" type="text" :placeholder="aiPlaceholders.owner_name || 'イオンペット 太郎'" :class="{ 'ai-placeholder': !formData.owner_name && aiPlaceholders.owner_name }" />
          </div>

          <div class="form-group" ref="field_favorite_word">
            <label>お好きな一言（免許の条件等）</label>
            <input v-model="formData.favorite_word" type="text" :placeholder="aiPlaceholders.favorite_word || '元気いっぱい！'" :class="{ 'ai-placeholder': !formData.favorite_word && aiPlaceholders.favorite_word }" />
          </div>
        ...
        <div class="special-notes-section" ref="field_special_notes">
          ...
          <div class="special-notes-grid">
            <div v-for="(note, index) in formData.special_notes" :key="index" class="special-note-item">
              <label>特記事項 {{ index + 1 }}</label>
              <div class="special-note-input">
                <select v-model="note.type" @change="onSpecialNoteTypeChange(index)">
                  <option value="">選択してください</option>
                  <option v-for="option in specialNoteOptions" :key="option" :value="option">{{ option }}</option>
        ...
        <div v-if="petInfo" class="pet-info">
          ...
          <div v-if="petInfo.extra_features" class="extra-features">
            <p v-if="petInfo.extra_features.expression"><strong>表情:</strong> {{ petInfo.extra_features.expression }}</p>
            <p v-if="petInfo.extra_features.posture"><strong>姿勢:</strong> {{ petInfo.extra_features.posture }}</p>
            <p v-if="petInfo.extra_features.fur_amount"><strong>毛量/毛質:</strong> {{ petInfo.extra_features.fur_amount }}</p>
            <p v-if="petInfo.extra_features.size"><strong>サイズ:</strong> {{ petInfo.extra_features.size }}</p>
            <p v-if="petInfo.extra_features.age_estimate"><strong>推定年齢:</strong> {{ petInfo.extra_features.age_estimate }}</p>
            <p v-if="petInfo.extra_features.other_traits && petInfo.extra_features.other_traits.length > 0">
              <strong>その他:</strong> {{ petInfo.extra_features.other_traits.join('、') }}
            </p>
          </div>
        </div>
      </section>
...
// 特記事項のデフォルト値
const defaultSpecialNotes = ['もふもふ', 'つぶらな瞳', 'マイペース', '良く寝る', '食欲旺盛']
...
const formData = ref({
  ...
  favorite_word: '',
  microchip_no: '',
  special_notes: defaultSpecialNotes.map(note => ({ type: note, custom: '' })) as SpecialNote[]
})
...
const getSpecialNotesArray = (): string[] => {
  return formData.value.special_notes.map(note => getSpecialNoteValue(note)).filter(v => v)
}
  ...
  try {
    ...
    try {
      ...
      const extraFeatures = petInfo.value.extra_features ? {
        ...
        age_estimate: petInfo.value.extra_features.age_estimate || undefined,
      ...
      console.warn('プロフィール自動生成に失敗しました。デフォルト値を使用します:', profileError)
    ...
    const errorMessage = error?.response?.data?.detail || error?.message || '処理に失敗しました'
...
const generateLicense = async () => {
  ...
  const effectivePetName = formData.value.pet_name || aiPlaceholders.value.pet_name || 'ポチ'
  const effectiveOwnerName = formData.value.owner_name || aiPlaceholders.value.owner_name || 'サンプル 太郎'
  const effectiveGender = formData.value.gender || aiPlaceholders.value.gender || 'オス'
  const effectiveFavoriteWord = formData.value.favorite_word || aiPlaceholders.value.favorite_word || '元気いっぱい！'

  // 種類と毛色もプレースホルダー値をフォールバック
  const effectiveBreed = formData.value.breed || aiPlaceholders.value.breed || petInfo.value.breed
  const effectiveColor = formData.value.color || aiPlaceholders.value.color || petInfo.value.color || 'ブラック'
...
const uploadLicense = async () => {
  if (!licenseBlob.value || !eventData.value || !petInfo.value || !selectedFile.value) return
  ...
  try {
    ...
    const response = await saveLicense({
      ...
      petName: formData.value.pet_name || aiPlaceholders.value.pet_name || 'ポチ',
      ownerName: formData.value.owner_name || aiPlaceholders.value.owner_name || 'サンプル 太郎',
      animalType: petInfo.value.animal_type,
      breed: formData.value.breed || aiPlaceholders.value.breed || petInfo.value.breed,
      color: formData.value.color || aiPlaceholders.value.color || petInfo.value.color,
      ...
      favoriteWord: formData.value.favorite_word || aiPlaceholders.value.favorite_word,
...
const reset = () => {
  ...
  formData.value = {
    ...
    favorite_word: '',
    microchip_no: '',
    special_notes: defaultSpecialNotes.map(note => ({ type: note, custom: '' }))
  }
  // プレースホルダーもリセット
